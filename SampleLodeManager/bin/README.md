...
The main script (`oshibacomyaku_Sample Lode Manager.lua`) prepends the matching `bin/<platform>/` path to `package.cpath` before generic `bin/?.dll` / `bin/?.so`.
...
2. 成功すると `bin/darwin64/` と `bin/darwin-arm64/` に `lsqlite3complete.so` がコミットされ、`oshibacomyaku_Sample Lode Manager.lua` の `@provides` に darwin 行が追記されます。
